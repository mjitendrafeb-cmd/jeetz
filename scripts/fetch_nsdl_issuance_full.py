...
Used only by send_nsdl_issuance_report.py (the standalone rating-band /
peer-pricing issuance report). This is intentionally a separate module from
scripts/fetch_nsdl_issuance.py, which is a different implementation used by
send_team_news.py (7:40 team mail) — the two have different return shapes
and must not be merged into one file.
