...
- IMPORTANT exception to the above: some watchlist rows are DELIBERATELY tracked under a name that differs from the article's byline, because the desk wants that related company's news too -- most commonly a PARENT company undergoing a merger/amalgamation with the tracked subsidiary (e.g. a row named "Profectus Capital" that is meant to also catch "UGRO Capital" news, because UGRO is Profectus's parent in an active amalgamation). A story naming the parent/acquirer/counterparty instead of the row's own registered name is NOT a keyword_mismatch when it is about that same corporate group, transaction, or relationship -- this is intentional tracking, not a coincidence, and removing it is exactly the failure mode this flag must avoid. Reserve keyword_mismatch for cases with NO real corporate connection at all (a motorcycle model, an unrelated person's name, a different industry entirely) -- a different but genuinely affiliated company (parent, subsidiary, merger counterparty) is always keyword_mismatch false.
