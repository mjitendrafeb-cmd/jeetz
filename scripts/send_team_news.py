...
- keyword_mismatch is a SEPARATE, narrower flag from the above, and it exists for ONE specific, narrow failure mode: a SHORT alias or acronym (roughly 2-5 letters -- things like "BoB", "BOI", "PFC", "UGRO", "REC", "CUB") that coincidentally collides with unrelated text -- a product/model name that happens to contain it, an unrelated person's given name, a different organisation entirely that happens to share the same initials. It is NOT a general "is this really material / really about this entity" judgment call -- that judgment belongs in credit_view/analyst_action, not here. If the item's connection to the entity is through its FULL registered name, a distinctive multi-word name, or a clearly-named subsidiary/parent/brand relationship (see the exception below), that is essentially never a coincidence and keyword_mismatch must be false -- full names are specific enough that "mismatch" should be rare, reserved for the short-alias case specifically. A story that is genuinely about the entity, however minor the news, is keyword_mismatch false even if credit_view is Neutral and analyst_action is No Action -- do not set keyword_mismatch true just because a story is immaterial, unfamiliar, or hard to place. When keyword_mismatch is true, the row is removed from the newsletter entirely, so only use it when you are confident the ONLY thing connecting this story to the entity is a short alias/acronym that happens to also mean something else -- if genuinely unsure, leave it false and rely on "Neutral | No Action" instead.
- IMPORTANT exception to the above: some watchlist rows are DELIBERATELY tracked under a name that differs from the article's byline, because the desk wants that related name's news too. This comes in two shapes, both keyword_mismatch false, never true: (1) a PARENT/acquirer/merger-counterparty company -- a different legal entity, but affiliated (e.g. a row named "Profectus Capital" also meant to catch "UGRO Capital" news, because UGRO is Profectus's parent in an active amalgamation; "Brookfield" news relevant to its subsidiary IndoStar Capital Finance is the same shape); (2) a REBRANDED CONSUMER/TRADE NAME of the exact same legal entity, not a different company at all -- e.g. "Fibe" is EarlySalary Services' own consumer-facing brand name, "KreditBee" is KrazyBee Services' own brand name, so a story about "Fibe" or "KreditBee" IS a story about that row's entity, just under the name customers actually see. Both shapes are intentional tracking, not a coincidence, and removing either is exactly the failure mode this flag must avoid. Reserve keyword_mismatch for cases with NO real connection at all (a motorcycle model, an unrelated person's name, a different industry entirely) -- a parent/subsidiary/merger-counterparty or a rebranded trade name of the SAME entity is always keyword_mismatch false.
